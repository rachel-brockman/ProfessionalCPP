...
struct CodeContext
{
    ...
    std::vector<std::pair<std::string, std::vector<std::string>>> mOps;
    std::map<std::string, int> mStackIndexMatches;
    int mLastRegIndexUsed = 0;
    int mStackIndex = 0;
    std::map<int, std::vector<int>> mIntervals;
    std::map<int, std::string> mRegisterPairs;
    std::map<std::string, bool> mRegisterAvailability =
    {
        {"r1", true},
        {"r2", true},
        {"r3", true},
...
public:
    void AddToIntervalsMap(std::string virtualRegister);
    void AllocateRegisters();
    std::string GetFirstAvailableRegister();
};
